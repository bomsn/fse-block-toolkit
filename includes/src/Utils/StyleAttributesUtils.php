...
namespace FSE\BlockToolkit\Utils;

/**
 * Class StyleAttributesUtils
 *
 * Helpers class used for getting class and style from attributes.
 */
class StyleAttributesUtils {
	...
	/**
	 * Get spacing gap css inline styles from attributes.
	 *
	 * @param array $attributes The block attributes.
	 *
	 * @return array The gap css styles for the block
	 */
	public static function get_block_gap_class_and_style( $attributes ) {
		...
		if ( isset( $attributes['style']['spacing']['blockGap'] ) ) {
			...
			// Handle potential array format for blockGap
			if ( is_array( $gap_value ) ) {
				$top  = isset( $gap_value['top'] ) ? $gap_value['top'] : '';
				$left = isset( $gap_value['left'] ) ? $gap_value['left'] : '';
			...
			if ( is_string( $gap_value ) && strpos( $gap_value, 'var:preset|spacing|' ) === 0 ) {
				$preset_name = str_replace( 'var:preset|spacing|', '', $gap_value );
				$gap_css     = "gap:var(--wp--preset--spacing--{$preset_name});";
			} else {
				// It's a custom value
				$gap_css = 'gap:' . esc_attr( $gap_value ) . ';';
			}
	...
	public static function get_classes_and_styles_by_attributes( $attributes, $properties = array(), $exclude = array() ) {
		$classes_and_styles = array(
			...
			'gap'              => self::get_block_gap_class_and_style( $attributes ),
		);
